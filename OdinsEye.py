...
def find_mismatches(site_access_df, rms_df):
    ...
    return grouped_df, mismatches_df
...
# Function to compare current alarms with site access
def find_current_alarms(site_access_df, current_alarms_df):
    current_alarms_df['SiteName_Extracted'] = current_alarms_df['Site'].apply(extract_site)
    mismatched_alarms = pd.merge(site_access_df, current_alarms_df, left_on='SiteName', right_on='SiteName_Extracted', how='right', indicator=True)
    return mismatched_alarms[mismatched_alarms['_merge'] == 'right_only']
...
if site_access_file and rms_file and current_alarms_file:
    ...
    if 'SiteName' in site_access_df.columns and 'Site' in rms_df.columns:
        # Find mismatches
        mismatches_df, detailed_mismatches = find_mismatches(site_access_df, rms_df)
        ...
        # Compare with current alarms
        alarm_mismatches = find_current_alarms(site_access_df, current_alarms_df)

        if not alarm_mismatches.empty:
            st.write("Mismatched Sites with Current Alarms:")
            st.table(alarm_mismatches[['SiteName', 'Alarm Time']])  # Modify according to your column names
        else:
