...
def style_table(df, na_color='lightblue', default_bg='lightgray'):
    def highlight_na(val):
        if pd.isna(val):
            return f'background-color: {na_color}'
        return ''

    def highlight_default(val):
        return f'background-color: {default_bg}'

    styled_df = df.style.applymap(highlight_na).applymap(highlight_default, subset=df.columns)
    return styled_df
...
def display_matched_sites(matched_df):
    ...
    selected_datetime = st.date_input("Filter Start Time after", pd.to_datetime('now')).strftime("%Y-%m-%d %H:%M:%S")
    matched_df = matched_df[matched_df['Start Time'] >= selected_datetime]
